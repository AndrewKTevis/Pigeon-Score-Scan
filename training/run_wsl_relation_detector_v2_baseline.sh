...
PREPARED_DIR="${PROJECT_DIR}/training_data/prepared/muse_omr_relation_detector_subset_v2"
IMAGES_DIR="${PROJECT_DIR}/training_data/prepared/muse_omr_scan_regions_stratified_complete_page_v6"
OUTPUT_DIR="${PROJECT_DIR}/training_data/models/muse-omr-relation-detector-v2-e4-stage1-20260801"
# V5 was trained only on the original training partition.  Unlike the final-refit
# V7 model, it has not seen the V2 internal test works and is therefore a valid
# shape-compatible backbone initializer for this model-selection diagnostic.
INITIAL_BACKBONE="${PROJECT_DIR}/training_data/models/muse-omr-scan-semantic-detector-v5-matcher35-25-ablation-e12-runtime2-20260730/model.best.pt"
...
for path in \
  ...
  "${INITIAL_BACKBONE}"; do
  if [[ ! -s "${path}" ]]; then
    echo "Relation-detector V2 input is missing: ${path}" >&2
    exit 1
  fi
done
...
exec "${PYTHON_BIN}" \
  ...
  --minimum-required-class-test-objects 50 \
  ...
  --runtime-stop-reason relation-detector-v2-independent-support-stage-gate \
  --device "${TRAIN_DEVICE}" \
