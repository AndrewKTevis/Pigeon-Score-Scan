...
OUTPUT_DIR="${PROJECT_DIR}/training_data/models/muse-omr-scan-semantic-detector-v6-class-aware-matcher-e12-runtime2-20260730"
...
RUNTIME_STOP_REASON="${SCORESCAN_RUNTIME_STOP_REASON:-class-aware-matcher-regression-recovery-window}"
...
if [[ ! -s "${INITIAL_MODEL}" || ! -s "${INITIAL_CATEGORIES}" ]]; then
  echo "Class-aware ablation initialization is incomplete: ${INITIAL_MODEL}" >&2
  exit 1
fi

# The v5 weights preserve the proven 0.35/0.25 gains. This bounded window tests
# whether restoring 0.50/0.40 assignment only for regressed classes recovers
# rhythmic and text precision without giving back long-span mark accuracy.
exec "${PYTHON_BIN}" \
