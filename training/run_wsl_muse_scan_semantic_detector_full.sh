...
OUTPUT_DIR="${1:-${PROJECT_DIR}/training_data/models/muse-omr-scan-semantic-detector-v4-complete-page-e12-b2-20260730}"
EXPECTED_OUTPUT_DIR="${PROJECT_DIR}/training_data/models/muse-omr-scan-semantic-detector-v4-complete-page-e12-b2-20260730"
INITIAL_MODEL="${2:-${PROJECT_DIR}/training_data/models/muse-omr-scan-semantic-detector-v3-stratified-replay-e8-b2-20260729/model.best.pt}"
INITIAL_CATEGORIES="${PROJECT_DIR}/training_data/prepared/openscore_lieder_train_1091_svg_regions_complete_page_overlap_consistent_deduplicated_v4/categories.json"
...
if [[ "${OUTPUT_DIR}" != "${EXPECTED_OUTPUT_DIR}" ]]; then
  echo "Refusing non-complete-page Muse model output: ${OUTPUT_DIR}" >&2
  exit 2
fi
...
if [[ ! -s "${PREPARED_DIR}/manifest.json" ||
  ...
  echo "Registered Muse OMR scan dataset is incomplete: ${PREPARED_DIR}" >&2
  exit 1
fi
if [[ ! -s "${INITIAL_MODEL}" || ! -s "${INITIAL_CATEGORIES}" ]]; then
  echo "Compatible audited semantic initialization is incomplete: ${INITIAL_MODEL}" >&2
  exit 1
fi

# The registered fine-tuning test split gates every class with at least 25
# in-domain objects. Lieder remains present as bounded replay, while the
# initialized recovery checkpoint retains the quartet corpus, instead of
# spending a separate 150k-sample synthetic-only pre-stage.
# Four scan-domain epochs are mandatory before the first acceptance decision;
# the separate 200+ work independent holdout remains unchanged.
# Three text roles lack 25 objects in this source-disjoint in-domain test
# (expressionText=4, scoreText=18, techniqueText=0). They therefore cannot
# receive statistically meaningful local class floors. They remain part of
# the global metrics and training data, and the independent 248-work holdout
# has 235/352/235 objects respectively and keeps their explicit 0.80 floors.
exec "${PYTHON_BIN}" \
