...
MODEL_VERSION = "scorescan-selection-risk-forest-6"
...
SCENARIO_WEIGHTS = {
    ...
    "measure-localized-clear-gain": 0.08,
    "measure-localized-common-error": 0.06,
    "measure-localized-partial-trap": 0.04,
}
MODEL_CONFIGS = (
...
def _scenario_distances(kind: str, rng: random.Random) -> tuple[float, float]:
    if kind in {"semantic-clear-gain", "exact-clear-gain", "exact-invalid-family-clear-gain", "localized-clear-gain", "measure-localized-clear-gain"}:
    ...
    elif kind in {"semantic-false-consensus", "exact-family-redundant-trap", "exact-invalid-family-trap", "exact-cross-family-trap", "template-better", "localized-confident-trap", "localized-partial-trap", "measure-localized-common-error", "measure-localized-partial-trap"}:
...
def _build_dataset(*, seed: int, groups: int, decisions_per_group: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, list[dict[str, object]]]:
    ...
    for group_id in range(groups):
        ...
        for decision_id in range(decisions_per_group):
            ...
            selection_kind = "exact_majority" if (kind.startswith("exact-") or kind.startswith("measure-localized-")) else "semantic_consensus"
            selected_distance, template_distance = _scenario_distances(kind, rng)
            selected_valid = kind not in {"invalid-selected", "localized-partial-trap", "measure-localized-partial-trap"} and rng.random() > 0.015 + 0.10 * selected_distance
            ...
            elif kind.startswith("measure-localized-"):
                candidate_count = rng.randint(4, 8)
            ...
            consensus_boost = 0.0
            if kind in {"semantic-false-consensus", "exact-family-redundant-trap", "exact-invalid-family-trap", "localized-confident-trap", "localized-partial-trap", "measure-localized-common-error", "measure-localized-partial-trap"}:
            ...
            elif kind in {"localized-clear-gain", "measure-localized-clear-gain"}:
            ...
            elif kind in {"localized-confident-trap", "localized-partial-trap", "measure-localized-common-error", "measure-localized-partial-trap"}:
            ...
            if kind == "measure-localized-clear-gain":
                selected_page_probability = template_page_probability
                selected_visual_probability = _clip(template_visual_probability + rng.uniform(-0.04, 0.08))
                selected_measure_probability = _clip(template_measure_probability + rng.uniform(0.12, 0.28))
                selected_event_probability = _clip(template_event_probability + rng.uniform(0.10, 0.24))
                selected_context_probability = _clip(template_context_probability + rng.uniform(0.02, 0.14))
                selected_ensemble_probability = _clip(template_ensemble_probability + rng.uniform(0.10, 0.24))
            elif kind == "measure-localized-common-error":
                selected_page_probability = template_page_probability
                selected_visual_probability = _clip(template_visual_probability - rng.uniform(0.10, 0.30))
                selected_measure_probability = _clip(template_measure_probability + rng.uniform(0.04, 0.18))
                selected_event_probability = _clip(template_event_probability - rng.uniform(0.10, 0.28))
                selected_context_probability = _clip(template_context_probability - rng.uniform(0.08, 0.25))
                selected_ensemble_probability = _clip(template_ensemble_probability + rng.uniform(0.03, 0.18))
            elif kind == "measure-localized-partial-trap":
                selected_page_probability = template_page_probability
                selected_measure_probability = rng.uniform(0.75, 0.96)
                selected_event_probability = rng.uniform(0.70, 0.94)
                selected_ensemble_probability = rng.uniform(0.86, 0.995)
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=ROOT.parent / "training" / "experiments" / "candidate_selection_risk_v6_model.json")
    parser.add_argument("--report", type=Path, default=ROOT.parent / "training" / "experiments" / "candidate_selection_risk_v6_report.json")
    ...
    for scenario in scenarios:
        ...
        scenario_metrics[scenario] = _selective(labels[test][mask], deployed_test[mask], v6_accepted[mask], gains[test][mask])
    ...
    report = {
        ...
        "frozen_test": {
            ...
            "v6_policy": _policy_metrics(labels, metadata, test, v6_accepted, gains),
