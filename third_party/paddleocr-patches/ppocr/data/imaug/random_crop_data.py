...
class RandomCrop(object):
    def __init__(
        ...
        **kwargs,
        ...
        if not 0.0 <= negative_crop_prob <= 1.0:
            raise ValueError("negative_crop_prob must be between 0 and 1")
        self.size = size
        self.max_tries = max_tries
        self.min_crop_side_ratio = min_crop_side_ratio
        self.keep_ratio = keep_ratio
        self.negative_crop_prob = negative_crop_prob

    def _random_region(self, height, width):
        crop_w_min = min(int(width * self.min_crop_side_ratio), self.size[0])
        crop_w_max = int(self.size[0] * 3)
        crop_w = (
            width
            if crop_w_min >= crop_w_max
            else min(random.randint(crop_w_min, crop_w_max), width)
        )

        crop_h_min = min(int(height * self.min_crop_side_ratio), self.size[1])
        crop_h_max = int(self.size[1] * 3)
        crop_h = (
            height
            if crop_h_min >= crop_h_max
            else min(random.randint(crop_h_min, crop_h_max), height)
        )

        crop_x = 0 if crop_w >= width else random.randint(0, width - crop_w)
        crop_y = 0 if crop_h >= height else random.randint(0, height - crop_h)
    ...
    def __call__(self, data):
        ...
        if len(all_care_polys) == 0:
            ...
            if (
                data.get("hard_negative_sampling_authorized") is True
                and random.random() < self.negative_crop_prob
            ):
                # Full-page score labels contain text, but most regions contain only
                # notation. Explicitly expose those hard negatives so the detector
                # learns not to classify notes, slurs, hairpins, or staff fragments
                # as text.
                for _attempt in range(self.max_tries):
                    crop_x, crop_y, crop_w, crop_h = self._random_region(h, w)
                    if all(
            ...
            if not negative_selected:
                # Pre-compute char heights (min side of min bounding rotated rect)
                # for all care boxes.
                char_heights = np.array(
                    [get_min_rotated_rect_side(poly) for poly in all_care_polys]
                )

                # Try to find a suitable positive crop region.
                valid_care_data = []
                for _attempt in range(self.max_tries):
                    crop_x, crop_y, crop_w, crop_h = self._random_region(h, w)

                    # Check each care text box, clip and validate simultaneously
                    # (computed once).
                    valid_care_data = []
                    for care_idx, (poly, char_height) in enumerate(
                        ...
                        clipped_area = cv2.contourArea(
                            clipped_poly.astype(np.float32)
                        )
                        if clipped_area < 80:
                            continue

                        # Validate - char height check
                        clipped_char_height = get_min_rotated_rect_side(
                            clipped_poly
                        )
                    ...
                    crop_x, crop_y, crop_w, crop_h = 0, 0, w, h
                    valid_care_data = [
                        (i, None) for i in range(len(all_care_polys))
                    ]
