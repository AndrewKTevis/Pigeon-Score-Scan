    ...
    if checkpoints:
        ...
        if os.path.exists(checkpoints + ".states"):
            ...
            if "global_step" in states_dict:
                # Chunked ScoreScan training must retain the absolute step for
                # evaluation scheduling, logs, and the next checkpoint.  The
                # optimizer/EMA states were already restored above; dropping
                # this top-level counter made otherwise-correct resumes restart
                # their audit trail at zero.
