...
def train(
    ...
    maximum_epochs_this_run = int(
        os.environ.get("SCORESCAN_MAX_EPOCHS_THIS_RUN", "0") or "0"
    )
    if maximum_epochs_this_run < 0:
        raise ValueError("SCORESCAN_MAX_EPOCHS_THIS_RUN cannot be negative")
    end_epoch = epoch_num
    if maximum_epochs_this_run:
        end_epoch = min(
            epoch_num,
            start_epoch + maximum_epochs_this_run - 1,
        )
        logger.info(
            "bounded resumable training invocation: epochs %d through %d of %d",
            start_epoch,
            end_epoch,
            epoch_num,
        )
