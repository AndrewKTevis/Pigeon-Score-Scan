...
from train_grace_patch_calibrator import (  # noqa: E402
    SAMPLES_PER_GROUP,
...
def test_grace_patch_training_groups_are_related_and_split_without_leakage() -> None:
...
def test_grace_patch_training_data_contains_positive_and_harmful_proposals() -> None:
    ...
    assert "remove-false-regular" in scenarios
    assert "restore-missing-regular" in scenarios
    assert "content-family-split" in scenarios
    assert "template-quality-superior" in scenarios
