...
from scorescan.articulation_consensus import (
    ArticulationPatchCalibrator,
    ArticulationPatchCandidate,
    ArticulationPatchInput,
    propose_articulation_patch,
)
from scorescan.articulation_xml import normalized_articulation_topology, set_articulation_topology
...
class AlwaysAccept:
    ...
    def calibrate(self, item: ArticulationPatchInput):
...
def _candidate(
    ...
    *,
    ...
    return ArticulationPatchCandidate(
        variant=variant,
        family=family,
        measure=measure,
        semantics=semantics,
...
def _propose(candidates: list[ArticulationPatchCandidate], *, base_measure: etree._Element | None = None):
    return propose_articulation_patch(
        candidates,
        template_index=0,
...
def test_articulation_patch_adds_and_removes_simple_markers() -> None:
    empty = ((), (), (), ())
    correct = (("staccato",), (), ("accent", "tenuto"), ())
    candidates = [
    ...
    assert result.changed_event_indices == (0, 2)
    assert result.changed_mark_count == 3
    assert result.patched_measure is not None
    assert normalized_articulation_topology(result.patched_measure.findall("note")) == correct
    ...
    assert normalized_articulation_topology(removed.patched_measure.findall("note")) == empty


def test_articulation_patch_uses_event_local_support_despite_other_pitch_error() -> None:
    empty = ((), (), (), ())
    correct = ((), ("staccato",), (), ())
    candidates = [
        _candidate("primary", "baseline", empty),
...
def test_articulation_patch_split_or_invalid_sibling_abstains() -> None:
    empty = ((), (), (), ())
    correct = (("staccato",), (), (), ())
    split = [
        _candidate("primary", "baseline", empty),
    ...
    assert _propose(split).reason == "no_articulation_change"
    ...
    assert _propose(invalid).reason == "no_articulation_change"


def test_articulation_patch_rejects_unsupported_or_complex_measure() -> None:
    empty = ((), (), (), ())
    correct = (("staccato",), (), (), ())
    unsupported = [
        _candidate("primary", "baseline", empty),
...
def test_articulation_patch_preserves_other_notation_and_child_order() -> None:
    empty = ((), (), (), ())
    correct = (("tenuto",), (), (), ())
    base = _measure(empty)
    ...
    set_articulation_topology(notes, correct)
    assert normalized_articulation_topology(notes) == correct


def test_articulation_patch_missing_model_and_alignment_gap_fail_closed(tmp_path: Path) -> None:
    empty = ((), (), (), ())
    correct = (("staccato",), (), (), ())
    candidates = [
        _candidate("primary", "baseline", empty),
    ...
    calibrator = ArticulationPatchCalibrator(tmp_path / "missing.json")
    assert not calibrator.enabled
    result = propose_articulation_patch(
        candidates,
        template_index=0,
        missing_candidate_count=0,
    ...
    gap = propose_articulation_patch(
        candidates,
        template_index=0,
...
def test_articulation_patch_detects_candidate_semantics_change() -> None:
    empty = ((), (), (), ())
    correct = (("staccato",), (), (), ())
