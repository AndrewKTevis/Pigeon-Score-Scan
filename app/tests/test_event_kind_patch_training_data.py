...
from train_event_kind_patch_calibrator import SCENARIO_WEIGHTS, _split, build_dataset  # noqa: E402
from scorescan.event_kind_consensus import FEATURE_NAMES  # noqa: E402


def test_event_kind_patch_training_data_is_deterministic_and_grouped() -> None:
    left = build_dataset(seed=20260721, groups=180)
    right = build_dataset(seed=20260721, groups=180)
    ...
    assert left.features.shape == (180, len(FEATURE_NAMES))
    assert set(int(value) for value in left.groups) == set(range(180))

    partitions = _split(left.groups, seed=20260721)
...
def test_event_kind_patch_training_data_covers_adversarial_scenarios() -> None:
    dataset = build_dataset(seed=20260721, groups=1400)
