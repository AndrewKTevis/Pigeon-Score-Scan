...
from train_attribute_patch_calibrator import SCENARIO_WEIGHTS, _split, build_dataset  # noqa: E402
from scorescan.attribute_consensus import ATTRIBUTE_KINDS, FEATURE_NAMES  # noqa: E402


def test_attribute_patch_training_data_is_deterministic_and_grouped() -> None:
...
def test_attribute_patch_training_data_covers_all_attributes_and_adversarial_scenarios() -> None:
    dataset = build_dataset(seed=20260720, groups=1200)

    assert set(dataset.scenarios) == set(SCENARIO_WEIGHTS)
    assert set(dataset.kinds) == set(ATTRIBUTE_KINDS)
