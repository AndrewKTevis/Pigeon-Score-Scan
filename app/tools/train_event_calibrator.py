...
"""Train the family-balanced CPU event candidate calibrator.

The dataset is split by complete source-measure identity into training, independent
probability calibration, model-selection audit, and frozen test partitions. Related
preprocessing variants never cross partitions. The released JSON is evaluated through
the dependency-free production runtime before it can be written.
"""
...
from event_training_data import EventDataset, build_dataset  # noqa: E402
from scorescan.event_calibration import FEATURE_NAMES, LEGACY_FEATURE_NAMES  # noqa: E402
...
MODEL_VERSION = "scorescan-event-forest-2"
MODEL_CONFIGS = (
...
def _fit_forest(
    ...
    calibrator.fit(raw.reshape(-1, 1), y[calibration])
    payload: dict[str, object] = {
        ...
        "training_distribution": (
            "complete seven-variant event ensembles grouped by preprocessing family"
        ),
    }
    return model, calibrator, payload


def _calibrated_sklearn_probabilities(
    model: RandomForestClassifier,
    calibrator: LogisticRegression,
    values: np.ndarray,
) -> np.ndarray:
    return calibrator.predict_proba(model.predict_proba(values)[:, 1].reshape(-1, 1))[:, 1]


def _sample_metrics(labels: np.ndarray, probabilities: np.ndarray, threshold: float = 0.5) -> dict[str, float | int]:
...
def _audit_score(
    ...
    return (
        float(group_metrics["top1_accuracy"]),
...
def _fit_ablation(
    ...
    return calibrator.predict_proba(
        model.predict_proba(dataset.legacy_features[test])[:, 1].reshape(-1, 1)
    )[:, 1]
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "src" / "scorescan" / "resources" / "event_calibrator.json",
    )
    parser.add_argument(
        "--report",
        type=Path,
        default=ROOT.parent / "training" / "event_calibrator_report_v2.json",
    )
    parser.add_argument(
        "--baseline-model",
        type=Path,
        default=ROOT.parent / "training" / "baselines" / "event_calibrator_v1.json",
    )
    ...
    for config in MODEL_CONFIGS:
        ...
        probabilities = _calibrated_sklearn_probabilities(
            model,
            calibrator,
            dataset.features[audit],
        )
        group_metrics = _group_top1(probabilities, dataset, audit)
        candidates.append(
            {
                "config": dict(config),
                "sample": _sample_metrics(dataset.labels[audit], probabilities),
                "decision": group_metrics,
                "selection_score": list(
                    _audit_score(probabilities, dataset.labels[audit], group_metrics)
                ),
            }
        )
        trained.append((model, calibrator, payload))

    best_top1 = max(float(item["decision"]["top1_accuracy"]) for item in candidates)
    top1_tied = [
        index
        for index, item in enumerate(candidates)
        if float(item["decision"]["top1_accuracy"]) == best_top1
    ]
    best_log_loss = min(
        float(candidates[index]["sample"]["log_loss"]) for index in top1_tied
    )
    # Prefer the smallest model when its independent-audit log loss is within 0.01
    # of the best model at the same group Top-1 accuracy. This explicit simplicity
    # rule keeps release resources compact without consulting the frozen test.
    compact_equivalents = [
        index
        for index in top1_tied
        if float(candidates[index]["sample"]["log_loss"]) <= best_log_loss + 0.01
    ]
    selected_index = min(
        compact_equivalents,
        key=lambda index: (
            int(candidates[index]["config"]["n_estimators"]),
            int(candidates[index]["config"]["max_depth"]),
            int(candidates[index]["config"]["min_samples_leaf"]),
        ),
    )
    model, calibrator, payload = trained[selected_index]
    selected_config = dict(candidates[selected_index]["config"])
    payload["training_groups"] = args.groups
    payload["selected_on"] = "independent grouped model-selection audit"

    sklearn_test = _calibrated_sklearn_probabilities(
        model,
        calibrator,
    ...
    baseline = StandardizedLogisticModel.load(
        args.baseline_model,
        "event_candidate_calibration",
        LEGACY_FEATURE_NAMES,
    )
    baseline_probabilities = np.asarray(
        [baseline.predict(row) for row in dataset.legacy_features[test]],
        dtype=np.float64,
    )
    ablation_probabilities = _fit_ablation(
        dataset,
        train,
        calibration,
        test,
    ...
    report = {
        ...
        "split_unit": (
            "source measure identity; all seven correlated preprocessing variants remain together"
        ),
        ...
        "model_selection_audit": candidates,
        ...
        "baseline_v1_same_frozen_test": {
            ...
            "sample": _sample_metrics(dataset.labels[test], baseline_probabilities),
            "decision": _group_top1(baseline_probabilities, dataset, test),
            "by_scenario": {
        ...
        "family_feature_ablation": {
            "description": (
                "same forest configuration and split using only the thirteen legacy all-peer features"
            ),
            "sample": _sample_metrics(dataset.labels[test], ablation_probabilities),
            "decision": _group_top1(ablation_probabilities, dataset, test),
            "by_scenario": {
                scenario: _group_top1(
        ...
        "deployment_parity": {
            ...
            "implementation": "dependency-free production random-forest JSON runtime",
        },
        "scope": (
            "grouped procedural event-candidate selection with preprocessing-family correlations; "
            "not end-to-end scan or MusicXML accuracy"
        ),
        "limitations": [
            "No large frozen real-scan candidate ensemble corpus is bundled.",
            "The model supplies only a bounded prior and cannot create consensus or edit events.",
        ],
    }
