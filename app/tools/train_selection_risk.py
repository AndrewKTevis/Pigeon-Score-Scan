...
MODEL_VERSION = "scorescan-selection-risk-forest-4"
...
SCENARIO_WEIGHTS = {
    ...
    "exact-family-redundant-trap": 0.10,
    "exact-cross-family-trap": 0.08,
...
def _scenario_distances(kind: str, rng: random.Random) -> tuple[float, float]:
    if kind in {"semantic-clear-gain", "exact-clear-gain", "localized-clear-gain"}:
    ...
    elif kind in {"semantic-false-consensus", "exact-family-redundant-trap", "exact-cross-family-trap", "template-better", "localized-confident-trap", "localized-partial-trap"}:
...
def _build_dataset(*, seed: int, groups: int, decisions_per_group: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, list[dict[str, object]]]:
    ...
    for group_id in range(groups):
        ...
        for decision_id in range(decisions_per_group):
            ...
            if kind == "exact-family-redundant-trap":
            ...
            consensus_boost = 0.0
            if kind in {"semantic-false-consensus", "exact-family-redundant-trap", "localized-confident-trap", "localized-partial-trap"}:
            ...
            item = SelectionRiskInput(
                ...
                selected_distance_to_medoid=_clip(rng.uniform(0.0, 0.02) + 0.06 * difficulty),
...
def main() -> None:
    ...
    parser.add_argument("--report", type=Path, default=ROOT.parent / "training" / "selection_risk_report_v4.json")
    parser.add_argument("--baseline-model", type=Path, default=ROOT.parent / "training" / "baselines" / "selection_risk_v3.json")
    ...
    parser.add_argument("--semantic-minimum-coverage", type=float, default=0.05)
    ...
    parser.add_argument("--confirmation-seed", type=int, default=20270503)
    ...
    test_corroborated = np.asarray(
        [bool(metadata[int(index)]["corroborated_exact_majority"]) for index in test],
        dtype=bool,
    )
    v4_accepted = np.where(
        test_kinds == "exact_majority",
        (deployed_test >= exact_threshold)
        | (
            test_corroborated
            & (deployed_test >= float(args.minimum_threshold))
        ),
        deployed_test >= semantic_threshold,
    )
    ...
    baseline_accepted = np.where(
        test_kinds == "exact_majority",
        (baseline_all[test] >= baseline_exact_threshold)
        | (test_corroborated & (baseline_all[test] >= float(args.minimum_threshold))),
        baseline_all[test] >= baseline_semantic_threshold,
    )
    ...
    ablation_accepted = np.where(
        test_kinds == "exact_majority",
        (ablation_test >= ablation_exact_threshold)
        | (
            test_corroborated
            & (ablation_test >= float(args.minimum_threshold))
        ),
        ablation_test >= ablation_semantic_threshold,
    )
    ...
    for scenario in scenarios:
        ...
        scenario_metrics[scenario] = _selective(labels[test][mask], deployed_test[mask], v4_accepted[mask], gains[test][mask])
    ...
    confirmation_corroborated = np.asarray(
        [bool(item["corroborated_exact_majority"]) for item in confirmation_metadata],
        dtype=bool,
    )
    confirmation_accepted = np.where(
        confirmation_kinds == "exact_majority",
        (confirmation_probabilities >= exact_threshold)
        | (
            confirmation_corroborated
            & (confirmation_probabilities >= float(args.minimum_threshold))
        ),
        confirmation_probabilities >= semantic_threshold,
    )
    ...
    confirmation_baseline_accepted = np.where(
        confirmation_kinds == "exact_majority",
        (confirmation_baseline_probabilities >= baseline_exact_threshold)
        | (confirmation_corroborated & (confirmation_baseline_probabilities >= float(args.minimum_threshold))),
        confirmation_baseline_probabilities >= baseline_semantic_threshold,
    )
    ...
    report = {
        ...
        "frozen_test": {
            ...
            "v4_policy": _policy_metrics(labels, metadata, test, v4_accepted, gains),
            "baseline_v3_policy_same_test": _policy_metrics(labels, metadata, test, baseline_accepted, gains),
        ...
        "independent_confirmation": {
            ...
            "baseline_v3_policy_same_confirmation": _policy_metrics(
                confirmation_labels,
                confirmation_metadata,
                np.arange(len(confirmation_labels), dtype=np.int64),
