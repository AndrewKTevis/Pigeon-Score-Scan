...
"""Train the CPU veto for conservative within-measure slur repairs."""
...
from scorescan.slur_consensus import FEATURE_NAMES, SlurPatchInput  # noqa: E402
...
MODEL_VERSION = "scorescan-slur-patch-forest-1"
...
SCENARIO_WEIGHTS = {
    "missing-phrase-strong-support": 0.17,
    "spurious-phrase-strong-support": 0.15,
    "short-phrase-strong-support": 0.09,
    "two-disjoint-phrases": 0.07,
    "context-supported-neutral": 0.07,
    "common-mode-false-phrase": 0.13,
    "weak-family-margin": 0.09,
    "visual-conflict": 0.07,
    "context-conflict": 0.06,
    "template-quality-superior": 0.06,
    "wide-scope-majority": 0.04,
}
POSITIVE_SCENARIOS = {
    "missing-phrase-strong-support",
    "spurious-phrase-strong-support",
    "short-phrase-strong-support",
    "two-disjoint-phrases",
...
def _row(seed: int, group_id: int, sample_id: int) -> tuple[SlurPatchInput, int, str]:
    ...
    winner = 4 if eligible_families == 4 and rng.random() < (0.52 if positive else 0.17) else 3
    winner = min(winner, voting_families)
    runner_up = max(0, voting_families - winner)
    template_support = runner_up
    total_events = rng.randint(4, 18)
    winner_arcs = 1
    added_arcs = 1 if rng.random() < 0.56 else 0
    removed_arcs = 1 - added_arcs
    changed_endpoints = 2
    minimum_span = rng.randint(1, min(5, total_events - 1))
    maximum_span = rng.randint(minimum_span, min(DEFAULT_POLICY.slur_patch_max_span_events, total_events - 1))

    quality = 0.92 if positive else 0.65
    visual_centre = 0.86 if positive else 0.56
    context_centre = 0.92 if positive else 0.55
    ensemble_delta = rng.uniform(0.12, 0.30) if positive else rng.uniform(-0.20, 0.07)
    score_margin = rng.uniform(7.0, 40.0) if positive else rng.uniform(-38.0, 12.0)

    if scenario == "spurious-phrase-strong-support":
        added_arcs, removed_arcs = 0, 1
    elif scenario == "short-phrase-strong-support":
        minimum_span = maximum_span = rng.choice((1, 2))
        quality = 0.89
        context_centre = 0.88
    elif scenario == "two-disjoint-phrases":
        winner_arcs = 2
        added_arcs, removed_arcs = 2, 0
        changed_endpoints = 4
        minimum_span = rng.choice((1, 2, 3))
        maximum_span = rng.randint(minimum_span, min(5, total_events - 1))
        quality = 0.88
    elif scenario == "context-supported-neutral":
        visual_centre = 0.69
        context_centre = 0.98
        quality = 0.85
    elif scenario == "common-mode-false-phrase":
        quality = 0.84
        visual_centre = 0.39
        context_centre = 0.43
        ...
        visual_centre = 0.13
        context_centre = 0.78
        ensemble_delta = rng.uniform(-0.12, 0.03)
    elif scenario == "context-conflict":
        quality = 0.81
        visual_centre = 0.78
        context_centre = 0.13
        ensemble_delta = rng.uniform(-0.13, 0.03)
    elif scenario == "template-quality-superior":
        quality = 0.72
        visual_centre = 0.64
        context_centre = 0.67
        ensemble_delta = rng.uniform(-0.27, -0.03)
        score_margin = rng.uniform(-45.0, -4.0)
    elif scenario == "wide-scope-majority":
        total_events = max(total_events, 8)
        winner_arcs = 2
        added_arcs = rng.choice((1, 2))
        removed_arcs = rng.choice((1, 2))
        changed_endpoints = 4
        minimum_span = rng.randint(1, 4)
        maximum_span = rng.randint(max(6, minimum_span), min(DEFAULT_POLICY.slur_patch_max_span_events, total_events - 1))
        quality = 0.53
        visual_centre = 0.43
        context_centre = 0.44
    ...
    item = SlurPatchInput(
        ...
        winner_arc_count=winner_arcs,
        minimum_arc_span=minimum_span,
        maximum_arc_span=maximum_span,
...
def _fit(dataset: Dataset, train: np.ndarray, calibration: np.ndarray, seed: int, config: dict[str, int]):
    ...
    payload: dict[str, object] = {
        ...
        "target": "slur-topology repair improves a fixed simple event sequence",
        "scope": "within-measure non-overlapping slur proposals already passing independent-family and deterministic topology guards",
    }
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "src/scorescan/resources/slur_patch_calibrator.json")
    parser.add_argument("--report", type=Path, default=ROOT.parent / "training/slur_patch_calibrator_report_v1.json")
    ...
    threshold = max(float(DEFAULT_POLICY.slur_patch_probability_floor), raw_threshold)
    ...
    if int(frozen_policy["false_accepts"]) or int(confirmation_policy["false_accepts"]):
        raise RuntimeError("release threshold produced a harmful automatic slur repair")
    ...
    report = {
        ...
        "policy_probability_floor": DEFAULT_POLICY.slur_patch_probability_floor,
