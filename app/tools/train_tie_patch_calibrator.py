...
"""Train the CPU veto for conservative within-measure tie repairs."""
...
from scorescan.tie_consensus import FEATURE_NAMES, TiePatchInput  # noqa: E402
...
MODEL_VERSION = "scorescan-tie-patch-forest-1"
...
SCENARIO_WEIGHTS = {
    "missing-pair-strong-support": 0.18,
    "spurious-pair-strong-support": 0.16,
    "three-note-chain": 0.08,
    "context-supported-neutral": 0.08,
    "common-mode-false-tie": 0.13,
    "weak-family-margin": 0.10,
...
POSITIVE_SCENARIOS = {
    "missing-pair-strong-support",
    "spurious-pair-strong-support",
    "three-note-chain",
...
def _row(seed: int, group_id: int, sample_id: int) -> tuple[TiePatchInput, int, str]:
    ...
    winner = 4 if eligible_families == 4 and rng.random() < (0.50 if positive else 0.18) else 3
    winner = min(winner, voting_families)
    runner_up = max(0, voting_families - winner)
    template_support = runner_up
    total_events = rng.randint(3, 12)
    changed_endpoints = 2 if rng.random() < (0.88 if positive else 0.53) else rng.choice((3, 4))
    changed_pairs = 1 if changed_endpoints <= 3 else 2
    added = changed_endpoints if rng.random() < 0.55 else 0
    removed = changed_endpoints - added
    winner_pairs = rng.randint(1, min(3, max(1, (total_events - 1) // 2)))

    quality = 0.91 if positive else 0.66
    visual_centre = 0.87 if positive else 0.57
    context_centre = 0.91 if positive else 0.56
    ensemble_delta = rng.uniform(0.11, 0.30) if positive else rng.uniform(-0.19, 0.08)
    score_margin = rng.uniform(6.0, 38.0) if positive else rng.uniform(-35.0, 12.0)

    if scenario == "spurious-pair-strong-support":
        added, removed = 0, changed_endpoints
    elif scenario == "three-note-chain":
        changed_endpoints = 3
        changed_pairs = 2
        added, removed = changed_endpoints, 0
        winner_pairs = max(2, winner_pairs)
        quality = 0.88
    elif scenario == "context-supported-neutral":
        visual_centre = 0.70
        context_centre = 0.97
        quality = 0.85
    elif scenario == "common-mode-false-tie":
        quality = 0.84
        visual_centre = 0.42
        context_centre = 0.47
        ...
        visual_centre = 0.60
        context_centre = 0.59
        ensemble_delta = rng.uniform(-0.10, 0.07)
        ...
        visual_centre = 0.76
        context_centre = 0.16
        ensemble_delta = rng.uniform(-0.12, 0.04)
    elif scenario == "template-quality-superior":
        quality = 0.72
        visual_centre = 0.66
        context_centre = 0.68
        ensemble_delta = rng.uniform(-0.26, -0.03)
        score_margin = rng.uniform(-42.0, -3.0)
    elif scenario == "wide-scope-majority":
        changed_endpoints = rng.choice((4, 5, 6))
        changed_pairs = rng.choice((2, 3))
        added = rng.randint(1, changed_endpoints)
        removed = changed_endpoints - added
        winner_pairs = rng.choice((2, 3, 4))
        quality = 0.54
        visual_centre = 0.45
        context_centre = 0.46
        ensemble_delta = rng.uniform(-0.21, 0.02)
    ...
    item = TiePatchInput(
        ...
        winner_tie_pair_count=winner_pairs,
...
def _fit(dataset: Dataset, train: np.ndarray, calibration: np.ndarray, seed: int, config: dict[str, int]):
    ...
    payload: dict[str, object] = {
        ...
        "target": "tie-endpoint repair improves a fixed simple event sequence",
        "scope": "within-measure monophonic tie proposals already passing independent-family and deterministic topology guards",
    }
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "src/scorescan/resources/tie_patch_calibrator.json")
    parser.add_argument("--report", type=Path, default=ROOT.parent / "training/tie_patch_calibrator_report_v1.json")
    ...
    threshold = max(float(DEFAULT_POLICY.tie_patch_probability_floor), raw_threshold)
    ...
    report = {
        ...
        "policy_probability_floor": DEFAULT_POLICY.tie_patch_probability_floor,
