...
"""Train the veto-only CPU calibrator for simple articulation repair."""
...
from scorescan.articulation_consensus import ArticulationPatchInput, FEATURE_NAMES  # noqa: E402
...
MODEL_VERSION = "scorescan-articulation-patch-forest-1"
...
SCENARIO_WEIGHTS = {
    "strong-staccato-majority": 0.16,
    "remove-spurious-accent": 0.12,
    "two-event-articulation-repair": 0.09,
    "context-supported-tenuto": 0.08,
    "visual-supported-accent": 0.08,
    "common-mode-dot-confusion": 0.12,
    "weak-family-margin": 0.08,
    "visual-conflict": 0.07,
    "context-conflict": 0.06,
    "template-quality-superior": 0.07,
    "cross-event-support-split": 0.07,
    "high-change-burden": 0.08,
}
POSITIVE_SCENARIOS = {
    "strong-staccato-majority",
    "remove-spurious-accent",
    "two-event-articulation-repair",
    "context-supported-tenuto",
    "visual-supported-accent",
}
...
def _row(seed: int, group_id: int, sample_id: int) -> tuple[ArticulationPatchInput, int, str]:
    ...
    visual_centre = 0.88 if positive else 0.50
    ...
    if scenario == "remove-spurious-accent":
        added, removed = 0, 1
    elif scenario == "two-event-articulation-repair":
        changed_events = 2
        added = rng.choice((1, 2))
        removed = 2 - added
        winner_marks = rng.choice((2, 3))
        max_marks = rng.choice((1, 2))
        quality = 0.88
    elif scenario == "context-supported-tenuto":
        visual_centre = 0.68
        context_centre = 0.98
        quality = 0.86
    elif scenario == "visual-supported-accent":
        visual_centre = 0.98
        context_centre = 0.67
        quality = 0.87
    elif scenario == "common-mode-dot-confusion":
        quality = 0.81
        visual_centre = 0.22
        context_centre = 0.39
        ensemble_delta = rng.uniform(-0.15, 0.02)
        ...
        quality = 0.71
        ensemble_delta = rng.uniform(-0.32, -0.05)
        score_margin = rng.uniform(-50.0, -6.0)
        visual_centre = 0.61
        context_centre = 0.62
    elif scenario == "cross-event-support-split":
        changed_events = 2
        winner = 3
        runner_up = 1 if voting_families == 4 else 0
        template_support = runner_up
        ...
        changed_events = rng.randint(4, 6)
        total_events = rng.randint(6, 10)
        added = rng.randint(2, changed_events)
        removed = changed_events - added
        winner_marks = changed_events + rng.randint(1, 3)
        max_marks = 2
        quality = 0.60
        visual_centre = 0.43
        context_centre = 0.46
    ...
    item = ArticulationPatchInput(
        ...
        added_mark_count=added,
        removed_mark_count=removed,
        minimum_winner_family_count=winner,
        mean_winner_family_count=float(winner) - rng.uniform(0.0, 0.18),
        minimum_winner_margin_count=max(0, winner - runner_up),
        mean_winner_margin_count=max(0.0, winner - runner_up - rng.uniform(0.0, 0.18)),
        maximum_template_family_count=template_support,
        incomplete_family_count=incomplete,
        winner_mark_count=winner_marks,
        maximum_marks_per_event=max_marks,
...
def _fit(dataset: Dataset, train: np.ndarray, calibration: np.ndarray, seed: int, config: dict[str, int]):
    ...
    payload: dict[str, object] = {
        ...
        "target": "simple articulation repair improves an aligned event lattice without changing notes or rhythm",
        "scope": "empty attribute-free accent/staccato/tenuto-family markers already passing strict family and XML guards",
    }
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "src/scorescan/resources/articulation_patch_calibrator.json")
    parser.add_argument("--report", type=Path, default=ROOT.parent / "training/articulation_patch_calibrator_report_v1.json")
    ...
    threshold = max(float(DEFAULT_POLICY.articulation_patch_probability_floor), raw_threshold)
    ...
    if int(frozen_policy["false_accepts"]) or int(confirmation_policy["false_accepts"]):
        raise RuntimeError("release threshold produced a harmful automatic articulation repair")
    ...
    report = {
        ...
        "policy_probability_floor": DEFAULT_POLICY.articulation_patch_probability_floor,
