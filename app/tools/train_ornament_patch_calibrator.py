...
"""Train the veto-only CPU calibrator for simple ornament repair."""
...
from scorescan.ornament_consensus import OrnamentPatchInput, FEATURE_NAMES  # noqa: E402
...
MODEL_VERSION = "scorescan-ornament-patch-forest-1"
...
SCENARIO_WEIGHTS = {
    "strong-trill-majority": 0.16,
    "remove-spurious-turn": 0.12,
    "two-event-ornament-repair": 0.09,
    "context-supported-mordent": 0.08,
    "visual-supported-trill": 0.08,
    "common-mode-ornament-confusion": 0.12,
...
POSITIVE_SCENARIOS = {
    "strong-trill-majority",
    "remove-spurious-turn",
    "two-event-ornament-repair",
    "context-supported-mordent",
    "visual-supported-trill",
}
...
def _row(seed: int, group_id: int, sample_id: int) -> tuple[OrnamentPatchInput, int, str]:
    ...
    if scenario == "remove-spurious-turn":
        added, removed = 0, 1
    elif scenario == "two-event-ornament-repair":
        changed_events = 2
        ...
        quality = 0.88
    elif scenario == "context-supported-mordent":
        visual_centre = 0.68
        context_centre = 0.98
        quality = 0.86
    elif scenario == "visual-supported-trill":
        visual_centre = 0.98
        context_centre = 0.67
        quality = 0.87
    elif scenario == "common-mode-ornament-confusion":
        quality = 0.81
        visual_centre = 0.22
        context_centre = 0.39
...
def _fit(dataset: Dataset, train: np.ndarray, calibration: np.ndarray, seed: int, config: dict[str, int]):
    ...
    payload: dict[str, object] = {
        ...
        "target": "simple ornament repair improves an aligned event lattice without changing notes or rhythm",
        "scope": "empty attribute-free trill/turn/mordent-family markers already passing strict family and XML guards",
    }
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "src/scorescan/resources/ornament_patch_calibrator.json")
    parser.add_argument("--report", type=Path, default=ROOT.parent / "training/ornament_patch_calibrator_report_v1.json")
    ...
    threshold = max(float(DEFAULT_POLICY.ornament_patch_probability_floor), raw_threshold)
    ...
    if int(frozen_policy["false_accepts"]) or int(confirmation_policy["false_accepts"]):
        raise RuntimeError("release threshold produced a harmful automatic ornament repair")
    ...
    report = {
        ...
        "policy_probability_floor": DEFAULT_POLICY.ornament_patch_probability_floor,
