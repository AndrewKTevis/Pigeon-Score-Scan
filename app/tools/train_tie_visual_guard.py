...
"""Train the verified CPU visual veto for within-measure tie additions."""
...
from tie_visual_training_data import TieVisualDataset  # noqa: E402
from tree_export import deployed_forest_probabilities, serialize_probability_forest  # noqa: E402

MODEL_VERSION = "scorescan-tie-visual-forest-1"
MODEL_CONFIGS = (
...
def _load_dataset(path: Path) -> TieVisualDataset:
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "src" / "scorescan" / "resources" / "tie_visual_guard.json",
    )
    parser.add_argument(
        "--report",
        type=Path,
        default=ROOT.parent / "training" / "tie_visual_guard_report_v1.json",
    )
    parser.add_argument("--seed", type=int, default=20260903)
    parser.add_argument("--train-data", type=Path, required=True)
    parser.add_argument("--safety-data", type=Path, required=True)
    parser.add_argument("--tail-safety-data", type=Path, nargs="+", required=True)
    parser.add_argument("--independent-data", type=Path, required=True)
    parser.add_argument("--ambiguity-data", type=Path, required=True)
    ...
    for config in MODEL_CONFIGS:
        ...
        local = _probabilities(model, calibrator, dataset.features[test])
        selection.append({"config": dict(config), "sample": _sample(dataset.labels[test], local)})
    ...
    threshold = max(
        threshold,
        _threshold(safety.labels, safety_probabilities, all_safety, MINIMUM_THRESHOLD),
    )

    tail_safety_parts = [_load_dataset(path) for path in args.tail_safety_data]
    tail_safety = TieVisualDataset(
        features=np.concatenate([item.features for item in tail_safety_parts], axis=0),
        labels=np.concatenate([item.labels for item in tail_safety_parts], axis=0),
        groups=np.concatenate(
            [
                item.groups + index * 10_000_000
                for index, item in enumerate(tail_safety_parts)
            ],
            axis=0,
        ),
        scenarios=tuple(
            scenario
            for item in tail_safety_parts
            for scenario in item.scenarios
        ),
    )
    tail_safety_probabilities = _probabilities(model, calibrator, tail_safety.features)
    all_tail_safety = np.arange(len(tail_safety.labels), dtype=np.int64)
    threshold = max(
        threshold,
        _threshold(
            tail_safety.labels, tail_safety_probabilities, all_tail_safety, MINIMUM_THRESHOLD
        ),
    )

    # The final independent set is deliberately loaded only after the threshold has
    # been frozen by the training split plus the two dedicated safety-calibration
    # sets.  It is never used for threshold or model selection.
    ...
    policies = {
        ...
        "safety_calibration": _policy(safety.labels, safety_probabilities, all_safety, threshold),
        "tail_safety_calibration": _policy(
            tail_safety.labels, tail_safety_probabilities, all_tail_safety, threshold
        ),
        "independent_test": _policy(
            independent.labels, independent_probabilities, all_independent, threshold
        ),
    }
    for name, policy in policies.items():
        if int(policy["false_accepts"]):
            raise RuntimeError(f"tie visual guard {name} false accepts: {policy}")
        if float(policy["present_recall"]) < MINIMUM_RECALL:
            raise RuntimeError(f"tie visual guard {name} recall too low: {policy}")

    payload = {
        "model_version": MODEL_VERSION,
        "model_type": "random_forest",
        "feature_names": list(TIE_VISUAL_FEATURE_NAMES),
        ...
        "model_config": config,
        "target": "a printed tie is present between one already-proposed adjacent same-pitch event pair",
        "scope": "veto-only tie-addition confirmation; removal and tie/slur type ambiguity are review-only",
    }
    ...
    # Candidate geometry alone is identical within each paired source group.  This
    # ablation must remain chance-level or the model has learned a semantic prior rather
    # than reading the preserved image.
    metadata_start = len(TIE_VISUAL_FEATURE_NAMES) - 2
    metadata_model, metadata_calibrator = _fit(
        dataset.features[:, metadata_start:],
        dataset.labels,
        train,
        calibration,
        args.seed,
    ...
    metadata_probabilities = _probabilities(
        metadata_model,
        metadata_calibrator,
        independent.features[:, metadata_start:],
    )

    ambiguity = _load_dataset(args.ambiguity_data)
    ambiguity_probabilities = _probabilities(model, calibrator, ambiguity.features)
    ambiguity_all = np.arange(len(ambiguity.labels), dtype=np.int64)
    ambiguity_policy = _policy(
        ambiguity.labels, ambiguity_probabilities, ambiguity_all, threshold
    )
    ...
    report = _canonical(
        {
            ...
            "model_version": MODEL_VERSION,
            "seed": args.seed,
            "scope": "programmatic rendered local tie-addition evidence only; not end-to-end OMR accuracy",
            ...
            "frozen_test": {
                ...
                "negative_scenarios": _scenario_false_accepts(
                    dataset, probabilities, threshold
                ),
            },
            "safety_calibration": {
            ...
            "tail_safety_calibration": {
                "datasets": [path.name for path in args.tail_safety_data],
                "groups": len(set(int(value) for value in tail_safety.groups.tolist())),
                "sample": _sample(tail_safety.labels, tail_safety_probabilities),
                "policy": policies["tail_safety_calibration"],
                "negative_scenarios": _scenario_false_accepts(
                    tail_safety, tail_safety_probabilities, threshold
                ),
            },
            "independent_test": {
            ...
            "ablation_geometry_only": {
                "feature_count": 2,
                "independent_sample": _sample(
                    independent.labels, metadata_probabilities
                ),
            },
            "rejected_same_endpoint_slur_experiment": {
                "deployed": False,
                "reason": "local source evidence could not prove tie-versus-slur type at the zero-false-accept boundary; explicit ambiguity is review-only",
                "groups": len(set(int(value) for value in ambiguity.groups.tolist())),
                "sample": _sample(ambiguity.labels, ambiguity_probabilities),
                "policy_at_deployment_threshold": ambiguity_policy,
            },
            "deployment_parity": {
