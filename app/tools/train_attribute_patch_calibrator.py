...
"""Train the CPU veto for conservative score-attribute repairs.

Rows represent proposals that already passed deterministic deployment guards: complete
preprocessing families, a strict independent-family majority, supported MusicXML values,
an observable attribute boundary, and meter non-regression for time signatures.  The
label asks whether changing one time/key/clef value improves the reference score without
introducing a wrong global interpretation.  The model can only reject a voted value.
"""
...
from scorescan.attribute_consensus import ATTRIBUTE_KINDS, FEATURE_NAMES, AttributePatchInput  # noqa: E402
...
MODEL_VERSION = "scorescan-attribute-patch-forest-1"
TARGET_PRECISION = 0.998
SCENARIO_WEIGHTS = {
    "clear-first-attribute": 0.15,
    "explicit-change-boundary": 0.16,
    "remove-spurious-template-change": 0.11,
    "meter-fit-repair": 0.13,
    "strong-context-repair": 0.09,
    "correlated-family-error": 0.11,
    "template-better": 0.08,
    "context-conflict": 0.07,
    "visual-conflict": 0.04,
    "edge-meter-ambiguity": 0.03,
    "weak-quality-majority": 0.03,
}
MODEL_CONFIGS = (
...
def _quality(rng: random.Random, centre: float, spread: float = 0.075) -> float:
...
def _row(seed: int, group_id: int) -> tuple[AttributePatchInput, int, str]:
    rng = random.Random((seed << 19) ^ (group_id * 0x9E3779B1))
    scenario = _choose_scenario(rng)
    kind = ATTRIBUTE_KINDS[group_id % len(ATTRIBUTE_KINDS)]
    positive = scenario in {
        "clear-first-attribute",
        "explicit-change-boundary",
        "remove-spurious-template-change",
        "meter-fit-repair",
        "strong-context-repair",
    }

    family_count = rng.choice((3, 4))
    candidate_count = family_count + rng.randint(1, 3)
    voting_families = family_count if rng.random() < 0.82 else max(3, family_count - 1)
    winner_count = voting_families if rng.random() < 0.45 else max(3, voting_families - 1)
    if family_count == 4 and scenario in {"correlated-family-error", "context-conflict", "visual-conflict"}:
        voting_families = 4
        winner_count = 3
    runner_up = max(0, voting_families - winner_count)
    winner_ratio = winner_count / family_count
    margin_ratio = max(1, winner_count - runner_up) / family_count
    template_ratio = runner_up / family_count
    abstention = (family_count - voting_families) / family_count

    first = scenario == "clear-first-attribute" or rng.random() < 0.12
    last = not first and rng.random() < 0.08
    template_explicit = scenario in {"remove-spurious-template-change", "explicit-change-boundary"} or rng.random() < 0.58
    support_explicit = rng.uniform(0.67, 1.0) if scenario == "explicit-change-boundary" else rng.uniform(0.0, 0.75)
    if first:
        support_explicit = rng.uniform(0.75, 1.0)
    if scenario == "remove-spurious-template-change":
        support_explicit = rng.uniform(0.0, 0.25)
        template_explicit = True

    previous_continuity = rng.uniform(0.76, 1.0) if positive else rng.uniform(0.25, 0.82)
    following_continuity = rng.uniform(0.78, 1.0) if positive else rng.uniform(0.20, 0.80)
    template_previous = rng.uniform(0.0, 0.48) if positive else rng.uniform(0.45, 1.0)
    template_following = rng.uniform(0.0, 0.48) if positive else rng.uniform(0.45, 1.0)

    template_meter_error = 0.0
    winner_meter_error = 0.0
    if kind == "time":
        if scenario == "meter-fit-repair":
            template_meter_error = rng.choice((0.5, 1.0, 1.5, 2.0))
            winner_meter_error = 0.0
        elif scenario == "edge-meter-ambiguity":
            first = True
            template_meter_error = rng.uniform(0.25, 1.5)
            winner_meter_error = max(0.0, template_meter_error - rng.uniform(0.0, 0.20))
        elif positive:
            template_meter_error = rng.choice((0.0, 0.5, 1.0))
            winner_meter_error = 0.0
        else:
            template_meter_error = rng.choice((0.0, 0.25, 0.5))
            winner_meter_error = rng.choice((0.0, 0.25, 0.5))
    meter_improvement = template_meter_error - winner_meter_error

    quality_centre = 0.88 if positive else 0.64
    visual_centre = 0.84 if positive else 0.61
    context_centre = 0.88 if positive else 0.56
    ensemble_delta = rng.uniform(0.08, 0.30) if positive else rng.uniform(-0.16, 0.09)
    score_margin = rng.uniform(5.0, 36.0) if positive else rng.uniform(-28.0, 14.0)

    if scenario == "correlated-family-error":
        quality_centre = 0.86
        visual_centre = 0.46
        context_centre = 0.42
        previous_continuity = rng.uniform(0.25, 0.62)
        following_continuity = rng.uniform(0.20, 0.58)
        support_explicit = rng.uniform(0.0, 0.38)
        ensemble_delta = rng.uniform(-0.04, 0.08)
    elif scenario == "template-better":
        quality_centre = 0.74
        visual_centre = 0.68
        context_centre = 0.72
        template_previous = rng.uniform(0.78, 1.0)
        template_following = rng.uniform(0.78, 1.0)
        previous_continuity = rng.uniform(0.40, 0.74)
        following_continuity = rng.uniform(0.40, 0.74)
        ensemble_delta = rng.uniform(-0.18, 0.02)
        score_margin = rng.uniform(-32.0, 4.0)
    elif scenario == "context-conflict":
        quality_centre = 0.84
        visual_centre = 0.78
        context_centre = 0.26
        previous_continuity = rng.uniform(0.05, 0.42)
        following_continuity = rng.uniform(0.05, 0.40)
        ensemble_delta = rng.uniform(-0.08, 0.08)
    elif scenario == "visual-conflict":
        quality_centre = 0.82
        visual_centre = 0.18
        context_centre = 0.66
        ensemble_delta = rng.uniform(-0.06, 0.06)
    elif scenario == "edge-meter-ambiguity":
        quality_centre = 0.78
        visual_centre = 0.61
        context_centre = 0.43
        ...
        score_margin = rng.uniform(-40.0, -6.0)
    ...
    event = _quality(rng, quality_centre + 0.02, 0.08)
    context = _quality(rng, context_centre, 0.10)
    ensemble = _quality(rng, quality_centre + 0.05, 0.07)
    minimum_ensemble = _clip(ensemble - rng.uniform(0.02, 0.20))

    # Overlap near the decision boundary prevents a single-feature shortcut and makes
    # threshold selection exercise the same high-precision veto regime as deployment.
    if rng.random() < 0.12:
        perturb = rng.gauss(0.0, 0.07)
        visual = _clip(visual + perturb)
        context = _clip(context - perturb / 2)
        following_continuity = _clip(following_continuity + rng.gauss(0.0, 0.08))
        ensemble_delta = _clip(ensemble_delta + rng.gauss(0.0, 0.05), -1.0, 1.0)

    item = AttributePatchInput(
        candidate_count=candidate_count,
        eligible_family_count=family_count,
        voting_family_count=voting_families,
        winner_family_support_ratio=_clip(winner_ratio + rng.uniform(-0.02, 0.02)),
        winner_family_margin_ratio=_clip(margin_ratio + rng.uniform(-0.03, 0.03)),
        template_family_support_ratio=_clip(template_ratio + rng.uniform(0.0, 0.04)),
        family_abstention_ratio=_clip(abstention + rng.uniform(0.0, 0.04)),
        attribute_kind=kind,
        is_first_measure=first,
        is_last_measure=last,
        template_has_explicit_attribute=template_explicit,
        support_explicit_attribute_ratio=_clip(support_explicit),
        support_previous_continuity_ratio=_clip(previous_continuity),
        support_following_continuity_ratio=_clip(following_continuity),
        template_previous_continuity=_clip(template_previous),
        template_following_continuity=_clip(template_following),
...
def build_dataset(seed: int, groups: int) -> Dataset:
    ...
    for group_id in range(groups):
        ...
        kinds.append(item.attribute_kind)
    return Dataset(
...
def _fit(
    ...
    payload: dict[str, object] = {
        ...
        "target": "single score-attribute patch improves the template without changing a correct time/key/clef",
        "scope": "strict independent-family attribute majorities already passing value, boundary and meter guards",
    }
...
def _select_threshold(labels: np.ndarray, probabilities: np.ndarray) -> tuple[float, dict[str, float | int]]:
    ...
    candidates.extend([0.92, 0.95, 0.975, 0.99, 0.995, 0.999, 0.9995])
...
def _slice_metrics(
    ...
    for label in sorted(set(labels[index] for index in indices)):
        local = np.asarray([offset for offset, index in enumerate(indices) if labels[index] == label], dtype=np.int64)
        result[label] = _metrics(dataset.labels[indices][local], probabilities[local], threshold)
    return result
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "src" / "scorescan" / "resources" / "attribute_patch_calibrator.json",
    )
    parser.add_argument(
        "--report",
        type=Path,
        default=ROOT.parent / "training" / "attribute_patch_calibrator_report_v1.json",
    )
    parser.add_argument("--seed", type=int, default=20260720)
    parser.add_argument("--groups", type=int, default=8000)
    parser.add_argument("--confirmation-groups", type=int, default=2400)
    ...
    report = {
        ...
        "frozen_test": {
            ...
            "scenarios": _slice_metrics(dataset, test, test_probabilities, threshold, dataset.scenarios),
            "attribute_kinds": _slice_metrics(dataset, test, test_probabilities, threshold, dataset.kinds),
        },
        "independent_confirmation": {
            ...
            "attribute_kinds": _slice_metrics(
                confirmation,
