...
"""Train the veto-only CPU calibrator for simple lyric repair."""
...
from scorescan.lyric_consensus import LyricPatchInput, FEATURE_NAMES  # noqa: E402
...
MODEL_VERSION = "scorescan-lyric-patch-forest-1"
...
SCENARIO_WEIGHTS = {
    "strong-lyric-majority": 0.16,
    "remove-spurious-lyric": 0.12,
    "two-event-lyric-repair": 0.09,
    "context-supported-syllable": 0.08,
    "visual-supported-syllable": 0.08,
    "common-mode-text-confusion": 0.12,
...
POSITIVE_SCENARIOS = {
    "strong-lyric-majority",
    "remove-spurious-lyric",
    "two-event-lyric-repair",
    "context-supported-syllable",
    "visual-supported-syllable",
}
...
def _row(seed: int, group_id: int, sample_id: int) -> tuple[LyricPatchInput, int, str]:
    ...
    if scenario == "remove-spurious-lyric":
        added, removed = 0, 1
    elif scenario == "two-event-lyric-repair":
        changed_events = 2
        added = rng.choice((1, 2))
        removed = 2 - added
        winner_marks = rng.choice((2, 3))
        max_marks = rng.randint(6, 24)
        quality = 0.88
    elif scenario == "context-supported-syllable":
        visual_centre = 0.68
        context_centre = 0.98
        quality = 0.86
    elif scenario == "visual-supported-syllable":
        visual_centre = 0.98
        context_centre = 0.67
        quality = 0.87
    elif scenario == "common-mode-text-confusion":
        quality = 0.81
        visual_centre = 0.22
        context_centre = 0.39
    ...
    item = LyricPatchInput(
        ...
        winner_lyric_count=winner_marks,
        changed_character_count=max_marks,
...
def _fit(dataset: Dataset, train: np.ndarray, calibration: np.ndarray, seed: int, config: dict[str, int]):
    ...
    payload: dict[str, object] = {
        ...
        "target": "simple lyric repair improves an aligned event lattice without changing notes or rhythm",
        "scope": "one plain lyric verse with syllabic/text/extend content already passing strict family and XML guards",
    }
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "src/scorescan/resources/lyric_patch_calibrator.json")
    parser.add_argument("--report", type=Path, default=ROOT.parent / "training/lyric_patch_calibrator_report_v1.json")
    ...
    threshold = max(float(DEFAULT_POLICY.lyric_patch_probability_floor), raw_threshold)
    ...
    if int(frozen_policy["false_accepts"]) or int(confirmation_policy["false_accepts"]):
        raise RuntimeError("release threshold produced a harmful automatic lyric repair")
    ...
    report = {
        ...
        "policy_probability_floor": DEFAULT_POLICY.lyric_patch_probability_floor,
