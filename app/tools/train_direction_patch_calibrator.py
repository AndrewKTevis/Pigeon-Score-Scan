...
"""Train the veto-only CPU calibrator for simple dynamic/metronome repair."""
...
from scorescan.direction_consensus import DirectionPatchInput, FEATURE_NAMES  # noqa: E402
...
MODEL_VERSION = "scorescan-direction-patch-forest-1"
...
SCENARIO_WEIGHTS = {
    "strong-dynamic-majority": 0.14,
    "remove-spurious-dynamic": 0.10,
    "add-metronome": 0.10,
    "replace-dynamic": 0.09,
    "mixed-expression-repair": 0.07,
    "common-mode-direction-confusion": 0.12,
    "weak-family-margin": 0.08,
    "visual-conflict": 0.07,
    "context-conflict": 0.06,
    "template-quality-superior": 0.07,
    "metronome-conflict": 0.05,
    "high-direction-burden": 0.05,
}
POSITIVE_SCENARIOS = {
    "strong-dynamic-majority",
    "remove-spurious-dynamic",
    "add-metronome",
    "replace-dynamic",
    "mixed-expression-repair",
}
...
def _row(seed: int, group_id: int, sample_id: int) -> tuple[DirectionPatchInput, int, str]:
    ...
    incomplete = 0 if rng.random() < 0.92 else 1
    base_directions = rng.choice((0, 1, 1, 2))
    winner_directions = max(0, base_directions + rng.choice((-1, 0, 1)))
    winner_directions = min(4, winner_directions)
    added = max(0, winner_directions - base_directions)
    removed = max(0, base_directions - winner_directions)
    if not added and not removed:
        added = 1
        winner_directions = min(4, base_directions + 1)
    changed = added + removed
    dynamic_changes = changed
    metronome_changes = 0
    max_per_onset = 1

    winner = 4 if eligible_families == 4 and rng.random() < (0.55 if positive else 0.10) else 3
    winner = min(winner, voting_families)
    runner_up = max(0, voting_families - winner)
    template_support = runner_up
    quality = 0.93 if positive else 0.63
    visual_centre = 0.87 if positive else 0.50
    context_centre = 0.88 if positive else 0.52
    ensemble_delta = rng.uniform(-0.04, 0.20) if positive else rng.uniform(-0.24, 0.05)
    score_margin = rng.uniform(-12.0, 30.0) if positive else rng.uniform(-44.0, 8.0)

    if scenario == "remove-spurious-dynamic":
        base_directions, winner_directions = 1, 0
        added, removed, changed = 0, 1, 1
    elif scenario == "add-metronome":
        base_directions, winner_directions = 0, 1
        added, removed, changed = 1, 0, 1
        dynamic_changes, metronome_changes = 0, 1
        visual_centre = 0.79
        context_centre = 0.94
    elif scenario == "replace-dynamic":
        base_directions, winner_directions = 1, 1
        added, removed, changed = 1, 1, 2
        dynamic_changes, metronome_changes = 2, 0
        quality = 0.89
    elif scenario == "mixed-expression-repair":
        base_directions, winner_directions = 1, 2
        added, removed, changed = 2, 1, 3
        dynamic_changes, metronome_changes = 2, 1
        quality = 0.87
        max_per_onset = rng.choice((1, 2))
    elif scenario == "common-mode-direction-confusion":
        quality = 0.80
        visual_centre = 0.20
        context_centre = 0.38
        ensemble_delta = rng.uniform(-0.16, 0.01)
        ...
        quality = 0.70
        ensemble_delta = rng.uniform(-0.34, -0.06)
        score_margin = rng.uniform(-52.0, -8.0)
        visual_centre = 0.60
        context_centre = 0.61
    elif scenario == "metronome-conflict":
        dynamic_changes, metronome_changes = 0, changed
        quality = 0.59
        context_centre = 0.39
        max_per_onset = 2
    elif scenario == "high-direction-burden":
        base_directions = rng.randint(2, 4)
        winner_directions = rng.randint(2, 4)
        added = rng.randint(2, 3)
        removed = rng.randint(1, 3)
        changed = added + removed
        dynamic_changes = rng.randint(1, changed)
        metronome_changes = changed - dynamic_changes
        quality = 0.57
        visual_centre = 0.42
        context_centre = 0.44
    ...
    item = DirectionPatchInput(
        ...
        changed_direction_count=changed,
        base_direction_count=base_directions,
        winner_direction_count=winner_directions,
        ...
        maximum_directions_per_onset=max_per_onset,
...
def _split(groups: np.ndarray, seed: int) -> tuple[np.ndarray, ...]:
    ...
    return tuple(
        np.flatnonzero(np.isin(groups, np.asarray(sorted(partition), dtype=np.int64)))
...
def _fit(dataset: Dataset, train: np.ndarray, calibration: np.ndarray, seed: int, config: dict[str, int]):
    ...
    payload: dict[str, object] = {
        ...
        "target": "simple dynamic/metronome repair improves expression directions without changing notes or rhythm",
        "scope": "compact dynamic tags and integer metronome marks already passing strict family, lattice and XML guards",
    }
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "src/scorescan/resources/direction_patch_calibrator.json")
    parser.add_argument("--report", type=Path, default=ROOT.parent / "training/direction_patch_calibrator_report_v1.json")
    ...
    threshold = max(float(DEFAULT_POLICY.direction_patch_probability_floor), raw_threshold)
    ...
    payload.update({
        ...
        "selected_on": "independent grouped model-selection audit with complexity preference",
    ...
    if int(frozen_policy["false_accepts"]) or int(confirmation_policy["false_accepts"]):
        raise RuntimeError("release threshold produced a harmful automatic direction repair")
    ...
    report = {
        ...
        "policy_probability_floor": DEFAULT_POLICY.direction_patch_probability_floor,
    ...
    report["model_bytes"] = args.output.stat().st_size
