...
"""Conservative event-level consensus for simple note articulations.

The repair votes independently per existing note and only changes empty, attribute-free
MusicXML articulation markers.  A family may vote for an event only when every sibling
...
from .articulation_xml import (
    ArticulationTopology,
    normalized_articulation_topology,
    set_articulation_topology,
    without_articulations,
)
...
class ArticulationPatchCandidate:
    variant: str
    family: str
...
class ArticulationPatchInput:
    ...
    def feature_vector(self) -> list[float]:
        ...
        return [
            ...
            unit(self.changed_event_count / max(1, DEFAULT_POLICY.articulation_patch_max_changed_events)),
            ...
            unit(self.winner_mark_count / max(1, changed * DEFAULT_POLICY.articulation_patch_max_marks_per_event)),
            unit(self.maximum_marks_per_event / max(1, DEFAULT_POLICY.articulation_patch_max_marks_per_event)),
...
@dataclass(frozen=True)
class ArticulationPatchCalibration:
    probability: float
    threshold: float
    accepted: bool
    model_version: str
...
class ArticulationPatchResult:
    ...
    accepted: bool
    reason: str
    input: ArticulationPatchInput | None = None
    model_version: str = "disabled"


class ArticulationPatchCalibrator:
    def __init__(self, model_path: Path | None = None) -> None:
        if model_path is None:
            model_path = Path(__file__).with_name("resources") / "articulation_patch_calibrator.json"
        loaded = load_verified_json(model_path, "articulation_patch_calibration")
        payload = loaded.payload
        self.model = VerifiedRandomForestModel.load(
            model_path,
            "articulation_patch_calibration",
            FEATURE_NAMES,
            loaded=loaded,
        )
        try:
        ...
        self.threshold = max(
            float(DEFAULT_POLICY.articulation_patch_probability_floor),
    ...
    def predict_probability(self, item: ArticulationPatchInput) -> float:
        return self.model.predict(item.feature_vector(), neutral=0.5)

    def calibrate(self, item: ArticulationPatchInput) -> ArticulationPatchCalibration:
        probability = self.predict_probability(item)
        return ArticulationPatchCalibration(
...
def _supported_measure_xml(measure: etree._Element, semantics: MeasureIR) -> bool:
    ...
    topology = normalized_articulation_topology(notes)
    if (
        ...
        or len(semantics.notes) > DEFAULT_POLICY.articulation_patch_max_events
    ...
    for note, node, marks in zip(semantics.notes, notes, topology, strict=True):
        if (
            ...
            or len(marks) > DEFAULT_POLICY.articulation_patch_max_marks_per_event
        ):
            return False
    return True


def _representative(items: Sequence[ArticulationPatchCandidate]) -> ArticulationPatchCandidate:
    return max(
        items,
        key=lambda item: (
...
def propose_articulation_patch(
    candidates: Sequence[ArticulationPatchCandidate],
    *,
    template_index: int,
    missing_candidate_count: int,
    calibrator: ArticulationPatchCalibrator | None = None,
    ...
    if not candidates or template_index < 0 or template_index >= len(candidates):
        return ArticulationPatchResult(None, (), 0, 0.5, 1.0, False, "invalid_input")
    if missing_candidate_count:
        return ArticulationPatchResult(None, (), 0, 0.5, 1.0, False, "alignment_gap")
    template = candidates[template_index]
    if not template.valid:
        return ArticulationPatchResult(None, (), 0, 0.5, 1.0, False, "invalid_template")
    ...
    if len(family_members) < DEFAULT_POLICY.articulation_patch_minimum_families:
        return ArticulationPatchResult(None, (), 0, 0.5, 1.0, False, "insufficient_families")
    ...
    if not _supported_measure_xml(source_base, base_semantics):
        return ArticulationPatchResult(None, (), 0, 0.5, 1.0, False, "unsupported_base_measure")
    base_notes = source_base.findall("note")
    base_topology = normalized_articulation_topology(base_notes)
    if base_topology is None:
        return ArticulationPatchResult(None, (), 0, 0.5, 1.0, False, "invalid_base_topology")

    candidate_topologies: dict[int, ArticulationTopology] = {}
    candidate_positions = {id(item): index for index, item in enumerate(candidates)}
    for index, item in enumerate(candidates):
        topology = normalized_articulation_topology(item.measure.findall("note"))
    ...
    family_representatives: dict[str, ArticulationPatchCandidate] = {}
    ...
    for event_index, base_note in enumerate(base_semantics.notes):
        ...
        representatives: dict[str, ArticulationPatchCandidate] = {}
        ...
        if len(votes) < DEFAULT_POLICY.articulation_patch_minimum_families:
            continue
        ...
        if not (
            winner_count >= DEFAULT_POLICY.articulation_patch_minimum_supporting_families
            and winner_count > len(votes) / 2
            and winner_count - runner_up >= 1
            and len(winner) <= DEFAULT_POLICY.articulation_patch_max_marks_per_event
        ):
            continue
    ...
    if not changed:
        return ArticulationPatchResult(None, (), 0, 0.5, 1.0, False, "no_articulation_change")
    if (
        len(changed) > DEFAULT_POLICY.articulation_patch_max_changed_events
        or len(changed) / max(1, len(base_semantics.notes)) > DEFAULT_POLICY.articulation_patch_max_changed_ratio
    ):
        return ArticulationPatchResult(None, changed, 0, 0.5, 1.0, False, "change_scope_too_large")

    common_support = set.intersection(*support_families_by_event)
    common_voting = set.intersection(*voting_families_by_event)
    if len(common_support) < DEFAULT_POLICY.articulation_patch_minimum_supporting_families:
        return ArticulationPatchResult(None, changed, 0, 0.5, 1.0, False, "inconsistent_cross_event_support")
    ...
    if changed_marks <= 0:
        return ArticulationPatchResult(None, changed, 0, 0.5, 1.0, False, "no_articulation_change")

    patched = copy.deepcopy(source_base)
    patched_notes = patched.findall("note")
    try:
        before_non_articulation = [without_articulations(note) for note in patched_notes]
        set_articulation_topology(patched_notes, proposal)
        after_non_articulation = [without_articulations(note) for note in patched.findall("note")]
    except ValueError:
        return ArticulationPatchResult(None, changed, changed_marks, 0.5, 1.0, False, "xml_topology_error")
    if before_non_articulation != after_non_articulation:
        return ArticulationPatchResult(None, changed, changed_marks, 0.5, 1.0, False, "unrelated_event_xml_changed")

    parsed, _ = measure_from_xml(patched, inherited)
    parsed_topology = normalized_articulation_topology(patched.findall("note"))
    if (
        ...
        return ArticulationPatchResult(None, changed, changed_marks, 0.5, 1.0, False, "post_patch_validation_failed")

    support = [family_representatives[family] for family in sorted(common_support)]
    item = ArticulationPatchInput(
    ...
    active_calibrator = calibrator or ArticulationPatchCalibrator()
    calibration = active_calibrator.calibrate(item)
    if not calibration.accepted:
        return ArticulationPatchResult(
            None,
            changed,
            changed_marks,
