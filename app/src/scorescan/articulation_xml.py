...
"""Canonical MusicXML helpers for simple note articulations.

The automatic repair path deliberately supports only empty, attribute-free articulation
markers whose semantics are represented completely by the element name.  More complex
MusicXML articulation content remains untouched and makes the local repair fail closed.
"""
...
ALLOWED_ARTICULATIONS = frozenset({
    "accent",
    "detached-legato",
    "spiccato",
    "staccatissimo",
...
ArticulationTopology = tuple[tuple[str, ...], ...]
...
def _simple_container(note: etree._Element) -> tuple[str, ...] | None:
    ...
    if not notations_nodes:
        return ()
    containers = notations_nodes[0].findall("articulations")
    if len(containers) > 1:
        return None
    if not containers:
        return ()
    ...
    for child in containers[0]:
        tag = str(child.tag)
        if (
            tag not in ALLOWED_ARTICULATIONS
            or child.attrib
            or len(child)
            ...
            return None
        result.append(tag)
    return tuple(sorted(result, key=lambda item: (_ARTICULATION_ORDER.get(item, 99), item)))


def normalized_articulation_topology(notes: Sequence[etree._Element]) -> ArticulationTopology | None:
    rows: list[tuple[str, ...]] = []
    for note in notes:
...
def set_articulation_topology(notes: Sequence[etree._Element], topology: Sequence[Sequence[str]]) -> None:
    if len(notes) != len(topology):
        raise ValueError("articulation topology length does not match note sequence")
    for note, requested in zip(notes, topology, strict=True):
        desired = tuple(sorted(set(requested), key=lambda item: (_ARTICULATION_ORDER.get(item, 99), item)))
        if any(item not in ALLOWED_ARTICULATIONS for item in desired):
            raise ValueError("unsupported articulation topology")
        notations_nodes = note.findall("notations")
        if len(notations_nodes) > 1:
            raise ValueError("multiple notations containers")
        notations = notations_nodes[0] if notations_nodes else None
        if notations is not None:
            for container in list(notations.findall("articulations")):
                notations.remove(container)
        if desired:
            if notations is None:
                notations = etree.Element("notations")
                _insert_note_child(note, notations)
            container = etree.Element("articulations")
            for tag in desired:
...
def without_articulations(note: etree._Element) -> bytes:
    clone = copy.deepcopy(note)
    topology = normalized_articulation_topology([clone])
    if topology is None:
        raise ValueError("unsupported articulation XML")
    set_articulation_topology([clone], [()])
